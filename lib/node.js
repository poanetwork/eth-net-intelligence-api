...
Node.prototype.reconnectWeb3 = function()
{
	...
	if(this.updateGasInterval)
		clearInterval(this.updateGasInterval);
	...
	if (this._web3 && (lastFetchAgo >= UPDATE_INTERVAL || forced === true))
	{
		...
		async.parallel({
			...
			gasPrice: function (callback)
			{
				...
				console.log('***** GET STATS.GASPRICE, self._lastGas = ' + JSON.stringify(self._lastGas));
				setTimeout(function () {
					//console.log('***** GET STATS.GASPRICE, in timeout: self._lastGas = ' + JSON.stringify(self._lastGas));
					//callback(null, new BigNumber(self._lastGas))
                                        console.log('***** USING MOCKED GASPRICE');
                                        callback(null, new BigNumber("1000000000"));

				}, 10);
				//web3.eth.getGasPrice(function (err,gp) { console.log('GASPRICE.err', err); console.log('GASPRICE.t', gp.constructor); console.log('GASPRICE.gp', gp); return callback(err,gp); });
			},
...
Node.prototype.getRealGas = function (forced) {
        console.log('***** MOCK REAL GAS *****');
        return;	
	console.log('***** GET REAL GAS *****');
	var self = this;
	var now = _.now();
	var lastGasFetchAgo = now - this._lastGasFetch;
	this._lastGasFetch = now;
	console.log('***** GET REAL GAS ***** lastGasFetchAgo = ' + lastGasFetchAgo);

	if (this._web3 && (lastGasFetchAgo >= UPDATE_GAS_INTERVAL || forced === true))
	{
		console.log('***** GET REAL GAS ***** calling RPC');
		console.stats('==>', 'Getting real gas stats')
		console.stats('   ', 'last update:', chalk.reset.cyan(lastGasFetchAgo));
		console.stats('   ', 'forced:', chalk.reset.cyan(forced === true));

		async.parallel({
			gasPrice: function (callback)
			{
				console.log('***** GET REAL GAS ***** making call');
				web3.eth.getGasPrice(function (err,gp) { return callback(err,gp); });
			}
		},
		function (err, results)
		{
			console.log('***** GET REAL GAS ***** in callback, err: ' + err);
			console.log('***** GET REAL GAS ***** in callback, results: ' + JSON.stringify(results));
			self._tries++;

			if (err) {
				console.error('xx>', 'getRealGas error: ', err);
			...
			results.end = _.now();
			results.diff = results.end - self._lastGasFetch;

			console.sstats('==>', 'Got getRealGas results in', chalk.reset.cyan(results.diff, 'ms'));

			if(results.gasPrice !== null)
			{
				console.log('***** GET REAL GAS ***** updating gasprice: ' + JSON.stringify(results.gasPrice));
				self._lastGas = results.gasPrice;
			}
			else {
				self.setInactive();
			}
...
Node.prototype.setWatches = function()
{
	...
	this.updateGasInterval = setInterval( function () {
		self.getRealGas();
	}, UPDATE_GAS_INTERVAL);
